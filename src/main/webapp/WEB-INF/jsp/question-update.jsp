...
<div class="container margin-bottom20px">
    ...
    <div class="row">
        <!-- 左边栏 -->
        <div class="col-md-8">
            <%--<input type="hidden" id="descriptionCh" value="${questionVo.questionDesc.descriptionCh}">--%>

            <c:if test="${sessionScope.userid == null}">
                <div class="alert alert-warning" role="alert">您还没有登录哦！无法提交修改的内容！</div>
            </c:if>

            <form action="/questions/${questionVo.id}/update" method="post" id="form">
                <div class="form-group">
                    <label>问题标题</label>
                    <input type="text" class="form-control" id="titleCh" name="titleCh" value="${questionVo.titleCh}">
                </div>
                <div class="form-group">
                    <label>问题描述</label>
                    <div id="wmd-button-bar" class="wmd-button-bar">
                    </div>
                    <textarea id="wmd-input" rows="22" class="wmd-input form-control" name="questionDesc.descriptionCh">${questionVo.questionDesc.descriptionCh}
                    </textarea>
                </div>
    ...
    $(document).ready(function(){
        ...
        $("#form").validate({
            rules:{
                ...
                'questionDesc.descriptionCh':{
                    required:true
                }
            },
            messages:{
                'titleCh':{
                    required:"<span style='color: red;'>问题标题不能为空</span>"
                },
                'questionDesc.descriptionCh':{
                    required:"<span style='color: red;'>问题描述不能为空</span>"
                }
            },
